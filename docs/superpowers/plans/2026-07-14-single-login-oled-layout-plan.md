...
1. 让 `loadAuthenticatedConsole()` 接受可选的登录首屏数据。
2. 正常登录时直接填充配置、渲染状态、连接事件流并关闭登录层。
3. 仅在已有会话的“重新连接”路径请求 `/config` 和 `/status`。
4. 浏览器断言一次密码提交后登录层关闭、无页面导航、登录接口只调用一次，且不立即请求配置/状态。
5. 保留错误密码、超时、401 会话失效和加载重试测试。
