...
2. OLED 显示热点名、持久化 WPA2 口令和配置地址。热点口令首次生成或通过 USB 配置后写入设备 secret，普通重启和固件部署不改变。
...
如果 OLED 自检失败，热点名、持久化口令和配置地址只输出到 USB 串口；设备保持电机关闭，直到完成首次配置或 OLED 恢复。热点口令以明文保存在设备 `/config/secrets.json` 中，因为 WPA2 接入点启动时必须取得原值；该文件不进入 Git、不通过网页 API 回显，完整擦除后需要重新生成或通过 USB 配置。

管理密码不保存原文，使用 16 字节随机盐和 PBKDF2-HMAC-SHA256 派生值。当前单用户可信局域网配置使用 2000 轮；密码记录保留迭代次数字段以兼容既有 20000 轮记录。首次设置和后续验证都采用异步分批计算，每 16 轮让出事件循环，避免阻塞 HTTP、SPI 心跳和状态广播。同一时间只运行一个密码计算，重复登录返回忙碌错误。设备所有者可以把热点口令和管理密码配置为同一值，但明文只通过设备本地配置流程和受控的首次 USB 输出处理，不写入源码、文档、持久日志或 Git 历史。
...
桌面与手机使用同一响应式页面。桌面端保留常驻左侧导航；手机端不使用可能被浏览器工具栏遮挡的底部导航，而是在顶栏提供始终可见的菜单按钮，打开带文字的左侧抽屉。抽屉包含全部四个页面和退出操作，点击页面、遮罩或按 ESC 后关闭，并正确维护 `aria-expanded`、焦点进入和焦点返回。移动视口使用 `100dvh` 和安全区内边距，保证菜单按钮、抽屉和表单操作不落入系统手势区。

在配置热点模式登录且家庭 Wi-Fi SSID 为空时，页面自动进入 `设置`；其他登录保持用户上次选择的默认控制台。设置页继续同时提供 Wi-Fi、OpenAI/DeepSeek、API Key 和运动限制，不拆分成一次性向导。静态资源随固件部署，不依赖 CDN。网页源码使用原生 ES 模块和精简的选定图标资源，构建产物写入 ESP 的 `/www` 目录。
...
- `/config/secrets.json`：管理密码派生值、热点口令和 API Key，仅由配置服务访问。
...
- 同步和异步 PBKDF2 结果一致；密码计算期间 SPI 心跳持续调度，并拒绝并发密码计算。
- STM 模拟器与 ESP SPI 邮箱网关的 TCP 槽集成。
- 模拟 LLM HTTP 服务与网页主要工作流。
- 网页桌面/移动视口、文本溢出、断线重连、急停和无障碍名称。
- 手机端顶栏菜单、侧边抽屉、遮罩/ESC 关闭、焦点返回、安全区适配和首次配网自动进入设置。
- 手机首次设置管理密码不出现 `Failed to fetch`，STM32 不因密码计算进入 ESTOP，热点口令重启后保持不变。
