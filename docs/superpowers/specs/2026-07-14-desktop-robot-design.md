...
- 任意运行状态收到本体长按、网页 STOP、会话建立后的 SPI 心跳超时或安全监督错误时进入 `ESTOP`。
- `ESTOP -> IDLE` 需要危险条件消失、长按按键已释放、SPI 链路健康，并在机器人本体再次短按确认。远程请求不能单独解除急停。
...
| 故障 | 立即行为 | 恢复 |
| --- | --- | --- |
| 会话建立后 ESP 失联或 SPI 心跳超时 | 进入 ESTOP、停轮、清队列、释放线圈 | 链路恢复且本体短按确认 |
